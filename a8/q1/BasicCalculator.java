...
public class BasicCalculator extends BaseFrame {
	...
	private JButton clearButton, resultsButton;
	...
	public void add(Container con) {
		...
		addButton(con,clearButton);
		addButton(con,resultsButton);
		addResultsLabel(con);
	...
	private void addButton(Container con,JButton button) {
		con.add(button);
	...
	void calculateAndStoreResult(){
		...
		String reusltanswer = ""+result;
		resultLabel.setText(reusltanswer);
	}

	private double getResult(double a, double b, String choice) {
		if (choice.equals("+")) {
			return a + b;
		}
		if (choice.equals("-")) {
	...
	public void addListeners() {
		...
		clearButton.addActionListener((e) -> new BasicCalculator());
	}
